...
│   │   └── agents/                 LangChain calls, one file per AI task (Gemini via get_gemini_llm(), or
│   │       │                        OpenRouter/Nemotron via get_openrouter_llm() — see "AI agents" below)
│   │       ├── _llm.py             get_gemini_llm()/get_openrouter_llm() — the ONE place model names are set
...
Most agents use `app/agents/_llm.py::get_gemini_llm()`, currently `gemini-flash-lite-latest`. **Do not
...
separate LLM factory alongside `get_gemini_llm()` — `ChatOpenAI` pointed at OpenRouter's OpenAI-compatible
...
agent stays on Gemini via `get_gemini_llm()`. `.with_structured_output(...)` works fine against it too
...
`langchain-core` `Runnable` (which is what `get_gemini_llm()`, `.with_structured_output(...)`, and
...
company, position, jd_text)` — same `get_gemini_llm().with_structured_output(...)` pattern and the same
...
Builder agent on **Gemini** (`get_gemini_llm()`) rather than OpenRouter, because extraction is

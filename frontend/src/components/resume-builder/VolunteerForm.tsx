import type { VolunteerEntry } from "../../lib/api";

function newEntry(): VolunteerEntry {
  return { id: crypto.randomUUID(), organization: "", role: "", start_date: "", end_date: "", description: "" };
}
...
export default function VolunteerForm({
  ...
  onChange: (entries: VolunteerEntry[]) => void;
}) {
  const update = (id: string, patch: Partial<VolunteerEntry>) =>
      ...
      {entries.map((entry, index) => (
        <div key={entry.id} className="subcard">
          ...
          <div className="form-row">
            ...
            <div className="field" style={{ flex: 1 }}>
              <label>Role</label>
              <input className="input" value={entry.role} onChange={(e) => update(entry.id, { role: e.target.value })} />
            </div>
          </div>
          <div className="form-row">
            <div className="field" style={{ flex: 1 }}>
